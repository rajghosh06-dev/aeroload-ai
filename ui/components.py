...
def render_aircraft_status(name: str, aircraft_id: str, bays: int) -> None:
    ...
    st.html(
        "<div class='header-aircraft'><small>ACTIVE AIRFRAME</small>"
        f"<strong>{escape(name)}</strong><span>{escape(aircraft_id)} · {bays} bays online</span></div>"
    )
...
def render_kpi_strip(
    *, item_count: int, bay_count: int, payload_kg: float, max_payload_kg: float,
    ...
    analysis_value = "CURRENT" if analysis_current else "DRAFT"
    analysis_note = "Results match inputs" if analysis_current else "Run solver to analyze"
    ...
    st.html(
        ...
        f"<small>{utilization:.0f}% of {max_payload_kg:,.0f} kg</small></article>"
        f"<article class='kpi-tile'><span>Remaining capacity</span><strong>{remaining:,.0f} kg</strong>"
        ...
        f"<article class='kpi-tile kpi-analysis'><span>Analysis</span><strong>{analysis_value}</strong>"
